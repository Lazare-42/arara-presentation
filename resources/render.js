...
function actionFromClick(clickedNode, dotSrc, index, svgNumber, graphviz, svg, currentGraphName) {
	...
	/**
	 *	You could open new graphs on click. Removed this possibility for this presentation
	 */
...
function attributer(datum, index, nodes, svg) {
	...
	if (datum.tag == "ellipse") {
		selection.attributes.fill = "red"
	}
	console.log(datum.tag)
	//nodes.selectAll(nodes).attr("color", "blue")
}
